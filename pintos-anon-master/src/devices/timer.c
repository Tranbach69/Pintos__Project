...
//    thread_yield ();
  ...
  /* Get current thread and set sleep ticks. */
  cur->wakeup_ticks = start + ticks;

  /* Insert current thread and set wakeup ticks. */
  list_insert_ordered (&sleeping_list, &cur->elem, thread_wakeup_ticks_cmp, NULL);
  thread_block ();
